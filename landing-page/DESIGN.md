...
colors:
  ...
  mute: "#7f9288"
  hairline: "#1d352a"
  hairline-strong: "#355446"
  canvas: "#06110d"
  canvas-soft: "#0a1711"
  canvas-soft-2: "#10231a"
  canvas-light: "#f7faf8"
  canvas-light-soft: "#e8f0eb"
  link: "#5bc2a1"
  link-deep: "#83d8bc"
  link-bg-soft: "#12382b"
  success: "#2f9e7f"
  ...
  warning-deep: "#ab570a"
  brand-green: "#2f9e7f"
  brand-green-bright: "#5bc2a1"
  brand-green-soft: "#d7eee5"
  brand-green-deep: "#0d3b2e"
  brand-green-dark: "#06110d"
  brand-glow: "rgba(47, 158, 127, 0.18)"
  gradient-hero-start: "#5bc2a1"
  gradient-hero-mid: "#2f9e7f"
  gradient-hero-end: "#0d3b2e"
  selection-bg: "#0d3b2e"
  selection-fg: "#ffffff"
...
### Brand & Accent
- **Deep Green** (`{colors.primary}` / `{colors.brand-green-deep}` — `#0d3b2e`): The primary CTA and branded section surface. It is dark enough to feel grounded but visibly greener than the page canvas.
- **Logo Green** (`{colors.brand-green}` — `#2f9e7f`): The canonical Bozon accent, aligned with the existing logo and implementation. Use it for active states, authored motion, focus, and key visual moments.
- **Bright Green** (`{colors.brand-green-bright}` — `#5bc2a1`): The accessible high-contrast accent for links and small marks on dark surfaces.
- **Brand Glow** (`{colors.brand-glow}`): Atmospheric support only. A glow never replaces a focus ring, border, or readable foreground.

### Surface
- **Canvas** (`{colors.canvas}` — `#06110d`): The default forest-black page surface.
- **Canvas Soft** (`{colors.canvas-soft}` — `#0a1711`): A subtle green-black lift for quiet bands and cards.
- **Canvas Soft 2** (`{colors.canvas-soft-2}` — `#10231a`): The elevated or inset surface for interactive regions and code previews.
- **Canvas Light** (`{colors.canvas-light}` — `#f7faf8`): An occasional editorial inversion, never the default page background.
- **Hairline** (`{colors.hairline}` — `#1d352a`): One-pixel separators and control borders on dark surfaces.
- **Hairline Strong** (`{colors.hairline-strong}` — `#355446`): Stronger control boundaries and focus-adjacent separation.

### Text
- **Ink** (`{colors.ink}` — `#f4f8f6`): Every heading and high-priority label on dark surfaces.
- **Body** (`{colors.body}` — `#b7c5bd`): Secondary text—sub-headings, body captions, nav-link inactive text, and footer copy.
- **Mute** (`{colors.mute}` — `#7f9288`): Lowest-priority text—placeholder text, fine print, and low-key labels.
- **On Primary** (`{colors.on-primary}` — `#ffffff`): All text on `{colors.primary}` surfaces.

### Semantic
- **Success** (`{colors.success}` — `#2f9e7f`): Positive state colour. Pair it with text, iconography, or position so colour is not the only signal.
- **Link** (`{colors.link}` — `#5bc2a1`): Accessible link and focus colour on dark surfaces; underline inline links.
- **Link Deep** (`{colors.link-deep}` — `#83d8bc`): Hover and emphasized-link tone.
- **Link Bg Soft** (`{colors.link-bg-soft}` — `#12382b`): Green-black informational and selected-state fill.
...
### Brand Gradient
The signature gradient follows the existing logo from mint highlight to forest depth:
- **Highlight** (`{colors.gradient-hero-start}` — `#5bc2a1`): The brightest visible point, used sparingly.
- **Core** (`{colors.gradient-hero-mid}` — `#2f9e7f`): The recognisable Bozon green and dominant moving colour.
- **Depth** (`{colors.gradient-hero-end}` — `#0d3b2e`): The dark end that blends naturally into the canvas.

Treat these stops as one monochromatic family. Shaders may vary lightness, opacity, scale, and density, but must not drift into blue, purple, pink, or orange. Keep the gradient atmospheric and large enough to read as light moving through depth rather than a decorative rainbow.
...
### Decorative Depth
- **Green gradient as atmospheric depth**: the hero's mint-to-forest field is the brand's only atmospheric colour effect—applied through the shader or as a flat 2-D backdrop rather than a generic 3-D illustration.
...
**`button-primary`** — the canonical 100-px-radius deep-green pill, marketing scale.
...
**`button-secondary`** — the quiet forest-black pill paired with the deep-green primary inside marketing bands.
...
**`button-secondary-sm`** — the smaller-scale quiet pill paired with `button-primary-sm`.
...
- Background `{colors.primary}`, text `{colors.on-primary}`, same shape + padding as `pricing-card`. CTA uses `button-secondary-sm` with an explicit hairline so it remains distinct.
...
- Text `{colors.link}` (`#5bc2a1`), body in `{typography.body-md}`, underlined.
...
### Do
- Reserve `{colors.primary}` (`#0d3b2e`) for primary CTAs and a small number of branded section surfaces. Deep green is the conversion target.
...
### Don't
- Don't introduce decorative blue, purple, pink, orange, or rainbow gradients. Bozon operates with off-white, forest-black, and one green family; red and amber are semantic exceptions only.
